...
code_seasonal = """
    ...
    vector[nreg] regressors[N]; // nreg regressors of N-time-steps
    matrix[12, 12] L_GP_prior; // prior covariance on regressor coefficients
    ...
    parameters {
    vector[12] b[nreg]; // regression coefficients
    ...
    vector[N] regression_model = rep_vector(0, N); // complete regression model
    
    // Model for the latent dynamical model
    trend[1] = x0;
    seasonal[1] = seasonal_components[1][1] + seasonal_components[1][3];
    for(i in 1:nreg){
    regression_model[1] += b[i][1]*regressors[1][i];
    }
    for(t in 2:N){
    ...
    for(i in 1:nreg){
    regression_model[t] += b[i][t - ((t-1)/12)*12]*regressors[t][i];
    }
    }
    }
    ...
    rho ~ uniform(0, 1);
    for(i in 1:nreg){
    b[i] ~ multi_normal_cholesky(rep_vector(0, 12), L_GP_prior);
    }
