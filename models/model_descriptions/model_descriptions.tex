...
\rot{dlm\_dynregs\_AR1} 
    \\ \hline
$\mu_t$ & \texttt{trend} & np.array(N) &  X   & X & X & X \\ 
$\alpha_t$ & \texttt{slope} & np.array(N) &  X   & X & X & X \\ 
$\beta_t$ & \texttt{beta} & np.array(N, $n_\mathrm{reg}$) &  X   & X &  & X \\ 
$u_{12,t} + u_{6,t}$ & \texttt{seasonal} & np.array(N) &  X  & X & X & X \\ 
$q_t$ & \texttt{ar} & np.array(N) &  X  & X & X & X \\ 
$\sigma_\mathrm{trend}$ & \texttt{sigma\_trend} & float &  X   & X & X & X \\ 
$\sigma_\mathrm{seas}$ & \texttt{sigma\_seas} & float &  X   & X & X & X \\ 
$\sigma_\mathrm{AR}$ & \texttt{sigma\_AR} & float &  X   & X & X & X \\ 
$\boldsymbol{\sigma}_{z}$ & \texttt{sigma\_reg} & np.array($n_\mathrm{reg}$) &    &  &  & X \\ 
$\rho_1$ & \texttt{rhoAR1} & float &  X   & X & X & X \\ 
$\rho_2$ & \texttt{rhoAR2} & float &   & X &  &  \\ 
 \hline
\end{tabular}
\end{center}
\end{table}
